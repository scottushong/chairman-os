...
import { removeKeyman, saveKeyman } from '@/app/actions/keymen'
import { Icon } from '@/components/ui/icon'
import { dDay } from '@/lib/format'
import type { BusinessKeyman } from '@/types'

/**
 * CH-024 확장 — 키맨 (0015 business_keymen).
 *
 * 이 회사를 움직이는 사람과, 마지막으로 언제 닿았나. 최근 접촉이 오래된 사람이 위로 온다 —
 * 회장이 이 패널을 여는 이유는 '누구에게 연락할 때가 됐나'다.
 *
 * 90일을 넘긴 접촉만 색을 준다(색은 위험에만). 기록이 없는 사람은 색 없이 '기록 없음'이다 —
 * 오래 연락 안 한 것과 기록을 안 한 것은 다르고, 둘을 같은 빨강으로 칠하면 기록하지 않는 편이 조용해진다.
 *
 * canEdit은 안내지 판정이 아니다. 0015의 business_keymen_write가 실제 문이다.
 */
...
const EMPTY: Draft = { name: '', relation: '', lastContactOn: '', note: '' }
...
export function KeymenPanel({
  ...
  businessId: string
  keymen: BusinessKeyman[]
  canEdit: boolean
}) {
  const [list, setList] = useState(keymen)
  ...
  async function submit() {
    ...
    setBusy(true)
    setError(null)
    const result = await saveKeyman({ ...draft, businessId })
    setBusy(false)
  ...
  async function remove(k: BusinessKeyman) {
    if (busy) return
    setBusy(true)
    setError(null)
    const result = await removeKeyman(k.keyman_id, businessId)
    setBusy(false)
    if (result.error) {
  ...
  return (
    <section className="rounded-xl border border-line-soft bg-panel p-4">
      ...
      {draft ? (
        <div className="mt-2.5 grid gap-2 rounded-lg bg-raised/60 p-2.5 md:grid-cols-4">
      ...
      {sorted.length === 0 && !draft ? (
        <p className="py-5 text-center text-[12px] text-ink-muted">
          등록된 키맨이 없습니다.
          {canEdit ? '' : ' 등록은 Chairman / Business CEO만 할 수 있습니다.'}
        </p>
      ) : (
