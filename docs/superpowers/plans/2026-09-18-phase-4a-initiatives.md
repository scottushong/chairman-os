...
- **흐리게(opacity)는 위험색을 덮지 않는다.** `text-critical` 요소의 **어느 조상도** 흐리게
  처리를 가질 수 없다. 흐리게는 물러나야 할 것(제목·메타·부가 문구·날짜 숫자)에만 건다.
  이 규칙을 두 번 어겼다 — Task 6의 줄 전체 `opacity-55`, Task 8의 칸 전체 `opacity-40`.
  두 번 다 하필 가장 급한 항목에서 빨강이 꺼졌다: 오래 방치된 건일수록 기한이 지나 있고,
  이번 달 밖 칸이야말로 지난 기한이 놓이는 자리다. 컨테이너에 opacity를 걸기 전에
  그 안에 색이 들어갈 수 있는지 먼저 본다.
