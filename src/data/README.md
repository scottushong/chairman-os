...
원본을 그대로 쓰면 화면이 비어서, 다음 네 가지만 손을 댔다. 되돌릴 때는 이 목록만 보면 된다.
...
4. **Task에 `blocked_since` 추가** (2026-09-05, Chairman 승인 · DEFERRED D-02 결정 A).
   시트 Tasks에는 `deadline`만 있어 CH-017 대기일수를 낼 수 없었다.
