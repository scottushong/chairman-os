...
import { isPeriodLocked, journalPeriods, lastClosedPeriod, slipsOf, todayKst } from '@/lib/ledger/journal'
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-line-soft pb-2">
            ...
            <MonthSelect
              ...
              hrefFor={Object.fromEntries(
                periods.map((p) => [p, withParams(basePath, { month: p === today.slice(0, 7) ? undefined : p })]),
              )}
            />
          </div>
          <SlipList slips={slips} accounts={new Map(accounts.map((a) => [a.account_code, a]))} />
        </section>
      </div>
    </div>
  )
